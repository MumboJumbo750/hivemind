...
- **Review-Gate gilt immer** — auch im Solo-Modus (Self-Review erlaubt, aber Schritt nicht überspringbar)
...
- **Guard-Reset bei `qa_failed → in_progress`:** Alle `task_guards`-Einträge für den Task werden auf `status = 'pending'` zurückgesetzt. Der Worker muss alle Guards erneut bestehen bevor er erneut `in_review` beantragen kann
